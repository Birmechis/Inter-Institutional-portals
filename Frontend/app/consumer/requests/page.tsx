...
interface APIRequest {
  ...
  providerResponse?: ProviderResponse; // <-- added
  apiEndpoint?: string;
...
export default function MyRequestsPage() {
  ...
  const openResponseModal = (response: ProviderResponse) => {
  ...
  const fetchEndpointData = async (response: any) => {
    if (!response?.endpointUrl) return;
    ...
    try {
      const res = await fetch(response.endpointUrl, {
        headers: {
                          ...
                          {request.providerResponse ? (
                            <div>
                              <Button
                                ...
                                onClick={() =>
                                  request.providerResponse &&
                                  openResponseModal(request.providerResponse)
                                }
                              >
                                View Response
                              </Button>

                              {loadingEndpoint && <div>Loading...</div>}
                              {endpointError && (
                              ...
                              {endpointData && (
                                <pre className="bg-gray-100 p-2 rounded max-h-48 overflow-auto text-xs">
        ...
        {isModalOpen && selectedResponse && (
          <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
            <div className="bg-white p-6 rounded shadow-lg w-96 max-w-full">
              <h2 className="text-lg font-semibold mb-2">Provider Response</h2>
              <p className="mb-2">
                <strong>Notes:</strong> {selectedResponse.notes || "No notes"}
              </p>
              <p className="mb-2">
                <strong>Sent At:</strong>{" "}
                {new Date(selectedResponse.sentAt).toLocaleString()}
              </p>
              <p className="mb-2">
                <strong>Endpoint:</strong>{" "}
                <a
                  href={selectedResponse.endpointUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 underline"
                >
                  {selectedResponse.endpointUrl}
                </a>
              </p>
              <p className="mb-4">
                <strong>Access Token:</strong> {selectedResponse.accessToken}
              </p>
