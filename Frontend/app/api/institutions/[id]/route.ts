...
import { NotificationService } from "@/lib/notificationcontext";
...
export async function PATCH(
  ...
  try {
    ...
    const success = await DatabaseService.updateInstitutionStatus(
      params.id,
      status
    );
    ...
    await DatabaseService.createAuditLog({
      userId: userId || "admin",
      action:
        status === "approved" ? "APPROVE_INSTITUTION" : "REJECT_INSTITUTION",
