...
export default function IncomingRequestsPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<MyRequest | null>(
    null
  );
  ...
  // open send info modal
  const handleSendInfo = (request: MyRequest) => {
    setSelectedRequest(request);
    setShowSendInfoModal(true);
  };

  // send info API call
  const handleSubmitSendInfo = async () => {
    if (!selectedRequest) return;

    try {
      const res = await fetch(
        `http://localhost:5000/api/requests/${selectedRequest.id}/send-info`,
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error("Failed to send info");

      alert("Information successfully sent to requester.");
      ...
      setHistory([
        {
          ...
          consumer: selectedRequest.institutionName,
          submittedDate: selectedRequest.createdAt,
          decisionDate: new Date().toISOString().split("T")[0],
          status: "info-sent", // optional: custom status for history
        },
        ...history,
      ]);

      setShowSendInfoModal(false);
      setResponseData({ endpointUrl: "", accessToken: "", notes: "" });
  ...
  return (
    <DashboardLayout userRole="provider">
      <div className="space-y-8">
                    ...
                    requests.map((request) => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap justify-end gap-3">
                          <Button
                            variant="outline"
                            onClick={() => handleSendInfo(request)}
                          >
                            Send Info
                          </Button>
        ...
        {/* Modal for Send Info */}
        {showSendInfoModal && selectedRequest && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg w-full max-w-lg space-y-4 shadow-lg">
              <h2 className="text-xl font-semibold mb-2">
                Send Information to {selectedRequest.institutionName}
              </h2>

              <p className="text-sm text-gray-600 mb-4">
                For request: <strong>{selectedRequest.title}</strong>
              </p>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full border rounded p-2"
                    placeholder="https://api.yourinstitution.com/data"
                    value={responseData.endpointUrl}
                    onChange={(e) =>
                      setResponseData({
                        ...responseData,
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full border rounded p-2"
                    placeholder="Provide access token or key"
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    className="w-full border rounded p-2"
                    placeholder="Include any usage instructions or additional details..."
              ...
              <div className="flex justify-end gap-2 mt-4">
                <button
                  className="px-4 py-2 bg-gray-200 rounded"
                  onClick={() => setShowSendInfoModal(false)}
                >
                  Cancel
                </button>
                <button
                  className="px-4 py-2 bg-blue-600 text-white rounded"
                  onClick={handleSubmitSendInfo}
                >
                  Send
                </button>
              </div>
